...
data class MySpot(
    ...
    /** 추천(좋아요) 수. 서버가 안 주면 null — 셀 메타 행에서 생략된다. */
...
data class MySpotDetail(
    ...
    /**
     * 소유권 — 서버 `isMySpot` 그대로다.
     * [source] 와 혼동하지 않는다: `source` 는 출처 표기(운영 큐레이션 / 유저 등록)이고
     * 소유권과 무관하다. 남이 올린 유저 스팟도 `SpotSource.User` 이면서 `isMySpot=false` 다.
     */
 ...
 * 상태를 바꾸는 요청의 공통 응답.
 *
 * `updatedAt` 은 없다 — 서버 응답 어디에도 그 필드가 없어(2026-08-26 OpenAPI 실측)
 * 낙관적 경합 토큰을 만들 수 없다. `docs/PV-41/09-api-mapping.md` B6 참고.
 */
...
interface MySpotService {
    ...
    /**
     * 공개(PUBLISHED) 스팟의 지도/리스트 노출 on/off. 검수 상태는 그대로라 재검수 없이 되돌릴 수 있다.
     * 반환은 서버가 확정한 노출 여부.
     */
    suspend fun setReleased(spotId: Long, released: Boolean): Boolean = unsupported("setReleased")
