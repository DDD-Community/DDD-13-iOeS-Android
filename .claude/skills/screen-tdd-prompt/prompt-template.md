# [{{ISSUE_REF}}] {{SCREEN_NAME}} 화면 구현 통합 프롬프트

> **이 프롬프트의 사용법**: `screen-tdd-prompt` 스킬이 이 템플릿을 복제·치환해서 `docs/{{ISSUE}}/{{SCREEN_SLUG}}-implementation-prompt.md`로 저장한다.
>
...
**브랜치**: `{{BRANCH}}` (이미 develop에서 분기/체크아웃됨)
**이슈**: {{ISSUE_URL}} ({{ISSUE_REF}})
...
**구현 범위**:
<!-- TODO: 이슈 본문의 acceptance criteria를 옮겨오기 (`gh issue view {{ISSUE_NUMBER}} -R do-ri/iOS`) -->
...
Phase B — ui-test-cases.md (Gate 1B + Gate 2)
  · 진입: Phase A 종료 조건 통과
  · 작업: docs/{{ISSUE}}/ui-test-cases.md 8컬럼 표 작성
...
`docs/{{ISSUE}}/{{SCREEN_SLUG}}-discussion.md` — 후속 합의 필요 항목. 결론이 필요하면 GitHub 이슈 {{ISSUE_REF}} 코멘트로 끌어올린다.
<!-- TODO: (a)/(b)/(c) 옵션 -->
...
- [ ] `docs/{{ISSUE}}/{{SCREEN_SLUG}}-discussion.md` 작성
- [ ] PR 본문에 `Closes {{ISSUE_REF}}` 포함 (머지 시 이슈 자동 종료)
...
```
0. §0~§8 합의 (이슈 본문 = `gh issue view {{ISSUE_NUMBER}} -R do-ri/iOS` 기준) → §9 에셋 매트릭스 채움 (Gate 4)
        ↓
...
3. docs/phases/phase-c-snapshot.md 읽기 → Phase C 수행 (Gate 1C + 3) → §11 Figma 루프
        ↓
4. §12 디버그 검증 → §13 논의 포인트 → §14 통과 → PR (`Closes {{ISSUE_REF}}`)
```
