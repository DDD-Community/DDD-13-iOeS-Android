...
| v1.0.2 | 1.0.2 | 1000201 | 준비 중 | 스팟 목록 페이지네이션 중복 key 크래시·중복 요청 방지, 스팟 상세 하단 내비게이션 바 가림 수정, 마이페이지 아바타 프로필 이미지 렌더링, 계정 관리 카메라 배지 아이콘 교체 |
| v1.0.3 | 1.0.3 | 1000301 | 준비 중 | **핫픽스.** v1.0.2 태그 기준(1.1.0 무드 필터 확장은 미포함). 비회원 스팟 신고 차단·나의 스팟 신고 진입점 제거, 북마크 연타 동시성 가드, 공지사항 하단 내비게이션 바 가림 수정, 탐색 리스트 무한 스크롤 복구 |
