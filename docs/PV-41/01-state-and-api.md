...
> **Stub 단계는 끝났다(2026-09-04).** 아래 Stub/Integration 2단계 서술은 진행 방식의 기록이고,
> `core/services/stub` 패키지는 전부 삭제됐다. 현재 계약은 `09-api-mapping.md` 가 단일 출처다.
...
상태 변경 응답은 `spotId`와 최종 `status`를 포함한다(공개 해제는 `previousStatus` 추가). 추천 응답은 최종 추천 수와 현재 사용자의 추천 여부를 포함한다.

> 서버 명세 확인 결과 상태 변경 응답에 **서버 갱신 시각은 없다**. 클라이언트 `MySpotTransitionResult.updatedAt`은 제거 대상이다([09-api-mapping.md](09-api-mapping.md) B6).

## 서버 선행 계약

| 항목 | 필요한 계약 | 상태 |
| --- | --- | --- |
| 상태값 | 다섯 상태의 정확한 wire 값 | ✅ 확정 — 클라 `MySpotStatus`와 완전 일치 |
| 동시 처리 | 철회와 승인/반려 경합 오류 코드, 최신 상태 응답 여부 | ✅ 확정 — 비관적 락, `409 SP004` |
| 반려 상세 | 반려 사유, 편집 필드, 기존 이미지 유지 방식 | ✅ 확정 — `rejection` 객체, 사유 enum 5종, 이미지 미첨부 시 유지 |
| 출처 | 큐레이션 소스 코드·표시명과 유저 등록 구분 | ✅ 확정 — `isCurated` |
| 추천 | 등록·취소 endpoint, 멱등성, 최종 카운트 응답 | ✅ 확정 — `/likes`, `SL001/SL002`, 응답 `likeCount`가 최종값 |
| 북마크 | 운영 삭제와 작성자 비공개를 구분하는 필드 | ✅ 확정 — `deleted` / `isPrivate` |
| 검수 결과 | 미확인 결과 조회·확인 완료·처리 중 신청 존재 여부 | ❌ **미확정** — 대응 엔드포인트 없음 |
| 승인 모달 | 승인 완료 모달 확인 여부 저장 위치 | ❌ **미확정** |
| 탈퇴 | 공개 스팟·추천 보존 및 재가입 복구 계약 | ❌ **미확정** |

임의 endpoint와 오류 코드를 클라이언트에서 먼저 확정하지 않는다.

## 공통 API 계약 입력란

출처: `pickflow API — 유저 스팟 공개 시스템` (2026.08.14), 서버 브랜치 `2026/HJY/유저스팟_공개시스템_개발`.
엔드포인트별 매핑과 잔여 갭은 [09-api-mapping.md](09-api-mapping.md)를 기준으로 한다.

| 항목 | 확정값 |
| --- | --- |
| Base URL / API version | context-path `/api` + `/v1` (dev `http://localhost:8080/api`) |
| 인증 방식 | `BearerAuth` (Authorization 헤더) |
| 공통 response envelope | `ApiResponse<T>` = `{ success, code, message, data }`, 성공 코드 `S000` |
| 공통 error envelope | 동일 envelope + 서비스 error code (`SP*`, `SL*`, `C*`). 인증 `401 C004`, 권한 `403 C005` |
| 날짜·시간 형식 / timezone | 날짜 `2024-05-01`, 시각 `18:30`, 타임스탬프 ISO-8601 UTC |
| 페이지 번호 기준 / page size | `page` 0-base, size **6 고정** |
| 상태 동시성 방식(version, updatedAt 등) | 서버 비관적 락 + 오픈 신청 이력 적재. 경합 시 `409 SP004`. **응답에 `updatedAt` 없음** |

비공개 스팟 접근은 `403`이 아닌 `404 SP001`로 응답해 존재 여부 자체를 숨긴다.

기능별 API 표에는 추측값을 넣지 않는다. 아직 계약이 없는 항목(검수 결과 알림 등)은 `TBD`로 두고 [09-api-mapping.md](09-api-mapping.md) C절에 명시한다.
