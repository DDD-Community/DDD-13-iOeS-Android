...
> **그대로 보낼 수 있는 요청서는 `docs/PV-59/backend-request.md` 에 있다.**
> 실측 근거·재현 명령·Slack 메시지 초안까지 포함돼 있으니 이 절 대신 그쪽을 쓰면 된다.
>
> ⚠️ **2026-08-16 재실측 결과 문서상 신규 2종이 운영에 아직 없다** —
> `?theme=SUNLIGHT` / `?theme=NIGHT_VIEW` 모두 400 `C002`.
> 즉 브랜치가 미배포 상태이며, 요청 항목이 "구현"이 아니라 "배포 일정 확인"이다.
...
- [ ] **(우선)** 백엔드에 요청 전달 — `docs/PV-59/backend-request.md` §5 메시지 그대로 사용
