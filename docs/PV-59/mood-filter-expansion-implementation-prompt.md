...
**범위 밖**:
- 무드 캡슐/칩 **컴포넌트 자체의 디자인 변경** — 패딩·아이콘 크기·타이포는 현행 유지.
  (Figma는 아이콘 20dp / `Body/large-bold` 17sp이지만 현재 코드는 16dp / `bodyMediumBold` 15sp다.
  이 격차는 이번 티켓에서 건드리지 않고 §13 논의 포인트로 넘긴다.)
...
**무드 캡슐 상태 (Figma `Category` 컴포넌트 세트 `1:43761`)**

| 상태 | 배경 | 보더 | 라벨 색 |
|---|---|---|---|
| Default | `#131416` | 없음 | `#FFFFFF` |
| Selected | `#131416` | `#FA6133` 1px | `#FFFFFF` |
| Disabled | `#464C53` | 없음 | `#8A949E` |

- 코너 8px, 패딩 8×14, 아이콘–라벨 간격 6px, 행 간격 8px (Figma 기준)
- **현재 코드는 패딩 14×6 / 간격 4px / 아이콘 16dp를 쓴다. 이번 티켓에서는 현행 유지**(§1 범위 밖)
- Disabled 상태는 현재 앱에 없다. 이번에도 도입하지 않는다

**등록 폼 칩 (Figma `Btn-tag` 컴포넌트 세트 `1:46095`)** — 배경 `#1E2124`, 패딩 8×12, 간격 6px, 코너 8px.
선택 시 `#FA6133` 1px 보더. 현행 구현과 동일하므로 값 추가만 한다.

**저장 탭 카드 배지 (Figma `Tag` 컴포넌트 세트 `1:43942`)** — 아이콘 박스 24×24(패딩 4, 코너 4, 배경 `#131416`) + 라벨 칩(패딩 4×8, 코너 4). 4종 변형 존재.
